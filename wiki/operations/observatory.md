...
## The sibling: Melch Obs

The provider-agnostic sibling repo, `../melch-obs`, reads this ledger and these run directories, drives the same bridge for syndicate targets, and adds bare-model probing and a signal viewer; it writes nothing back. When to use which, and what it depends on here: [Melch Obs](/operations/melch-obs.md) and [ADR 0013](/decisions/0013-melch-obs-sibling.md).
